...
- Chat messages for `FEISHU_CHAT_IDS`, or all reachable chats when unset.
- Calendar events for `FEISHU_CALENDAR_ID`, or the first reachable calendar when unset.

Each source reports failure independently. Failed sources retain their previous cache; successful Bitable/message/calendar snapshots replace their source transactionally. The dashboard shows the last collection time and any warnings. Detailed errors go to server logs. The timeline derives task events from current tasks, so local completion/deletion immediately updates it.
